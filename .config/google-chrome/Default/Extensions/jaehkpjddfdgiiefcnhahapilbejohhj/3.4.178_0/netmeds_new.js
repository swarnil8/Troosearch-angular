var alertPosition = 2238;
var imgLogo = chrome.extension.getURL("logo.png");

function reportPurchase(){
  var curURL = window.location.href;
  if(curURL.split('.netmeds.com/shopping-cart').length>1){
    var jsonArr = [{'processDONE': "Netmeds"}];
...
pidFlipkart = getPID();
var selector = [];
selector.push({selector: '.otc_prod_details:eq(0)', attr: 'none', pos: 'before'});
selector.push({selector: '#divRelatedProduct', attr: 'parent', pos: 'before'});
