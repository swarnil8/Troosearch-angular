...
function messageReceived(message) {
  ...
  else if(typeof(msgReceived[0].luckyWinner) != undefined && typeof(msgReceived[0].luckyWinner) != "undefined"){
...
chrome.notifications.onButtonClicked.addListener(function(notifID,buttonIndex){
 if(hashPushLink[notifID] && buttonIndex==0){
   ...
   targetURL = "http://options.buyhatke.com";
...
function getAffCoupons(coupIndex, tabID){
  if(coupIndex == 2){ 
    ...
    chrome.tabs.create({
     url : "https://tracking.buyhatke.com/Navigation/?pos=421&source=extension&ext1=auto-coupon&ext2=" + localStorage.ext_id + "&link=" + encodeURIComponent("http://www.shopclues.com/")
   });
  }
    ...
    chrome.tabs.create({
     url : "http://tracking.buyhatke.com/Navigation/?pos=63&source=extension&ext1=auto-coupon&ext2=" + localStorage.ext_id + "&link=" + encodeURIComponent("http://www.amazon.in/?tag=buyhatke-21")
   });
  }
...
function sendPush(key){
 ...
 if(pos==63 && d <= 28){
  ...
  switch(pos){
    ...
    if(url==""){
     url = "https://www.flipkart.com/offers?otracker=nmenu_offer-zone";
   }
   break; 
   case 63:
   var icon = "https://s3-ap-southeast-1.amazonaws.com/push-images/amazon-greatindiansale-notif.png";
   if(url==""){
     url = "https://www.amazon.in/b/?ie=UTF8&node=3419926031";
   }
   break;
   case 129:
   ...
   break; 
   case 50:
   var icon = "https://s3-ap-southeast-1.amazonaws.com/push-images/jabong-JFFGeneric-notify.png";
   if(url==""){
     url = "http://www.jabong.com/all-products/discount-more-than-29/new-products/?promotion=sale10offer&voucherCode=SALE10&cmpgp=newarrivalmin30";
   }
   break; 
   case 111:
   var icon = "https://s3-ap-southeast-1.amazonaws.com/push-images/myntra-BBDGeneric-notify.png";
   if(url==""){
     url = "http://www.myntra.com/flat-70-sale";
   }
   break; 
   case 421:
   var icon = "https://s3-ap-southeast-1.amazonaws.com/push-images/shopclues-DSGeneric-notify.png";
   if(url==""){
     url = "http://www.shopclues.com/diwali-offers.html";
   }
   break; 
 }
 if(url!="" && icon!=""){
  url = "https://tracking.buyhatke.com/Navigation/?pos=" + pos + "&source=extension&link=" + encodeURIComponent(url) + "&ext1=offers-update&ext2=" + localStorage.ext_id;
...
function monitorUser(){
 ...
 var d = new Date();
 d = d.getDate();
 if(Math.floor(Date.now() / 1000)-globalPush>1800 && d >20){
...
else if(Math.floor(Date.now() / 1000)-globalPush>1800){
  for (var key in visited) {
    var lastSeen = Math.floor(visited[key]);
    if(key.split("vis").length>1){
     key = key.split("vis")[0];
    ...
    if (request.message == "user_details"){
      ...
      console.log("entered promoPushArray with ",localStorage.promoPushArray);
...
function deleteEntries(arr, id) {
  ...
  port.onMessage.addListener(function(msg) {
    if (msg.messageData != ""){
      ...
      switch(command){
       case 1:
       var myurl = "http://aws1.buyhatke.com/extension/getPredictedData.php";
       break;
       case 2:
