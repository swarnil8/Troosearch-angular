var alertPosition = 2190;

function reportPurchase(){
  var curURL = window.location.href;
  if(curURL.split('tatacliq.com/cart').length>1){
    var jsonArr = [{'processDONE': "Tatacliq"}];
...
pidFlipkart = getPID();
var selector = [];
selector.push({selector: '.tabs-block:eq(0)', attr: 'none', pos: 'before'});
selector.push({selector: '.product-info:eq(0)', attr: 'none', pos: 'after'});
